const CACHE_NAME = "adis-world-v76";
const APP_SHELL = [
  "./", "./index.html",
  "./styles.css?v=74", "./drawing-garden.css?v=37", "./outfit-check.css?v=63", "./outfit-check-layered.css?v=70",
  "./app.js?v=74", "./drawing-garden.js?v=37", "./outfit-check.js?v=70",
  ...
  "./assets/icons/icon.svg", "./assets/icons/adis-world-splash.jpg", "./assets/worlds/word-forest-card.svg?v=74",
...
async function networkFirst(request) { try { const response = await fetch(request, { cache: "no-store" }); if (response && response.ok) { const cache = await caches.open(CACHE_NAME); await cache.put(request, response.clone()); } return response; } catch { return (await caches.match(request)) || (await caches.match("./index.html")); } }
async function cacheFirst(request) { const cached = await caches.match(request); if (cached) return cached; try { const response = await fetch(request); if (response && response.ok) { const cache = await caches.open(CACHE_NAME); await cache.put(request, response.clone()); } return response; } catch { return caches.match("./index.html"); } }
self.addEventListener("fetch", (event) => { if (event.request.method !== "GET") return; const url = new URL(event.request.url); const isSameOrigin = url.origin === self.location.origin; const isCoreRequest = isSameOrigin && (event.request.mode === "navigate" || CORE_PATHS.has(url.pathname)); event.respondWith(isCoreRequest ? networkFirst(event.request) : cacheFirst(event.request)); });
